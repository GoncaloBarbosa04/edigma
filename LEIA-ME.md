...
## O que falta e é preciso decidir

- Cores da marca — as do protótipo são aproximações tiradas do logótipo.
  É preciso o manual de identidade dos TUB.
- Tipografia — está a usar a fonte do sistema. Uma face com bons algarismos
  tabulares melhora bastante a leitura dos tempos.
- Mapa da rede — está representado por um marcador de posição. Decidir entre
  imagem estática, diagrama SVG ou mapa interativo muda bastante o esforço.
- Formato dos criativos publicitários — resolução, duração máxima de vídeo e
  se são aceites com ou sem áudio.
- Proof-of-play — o modelo de registo condiciona o contrato com anunciantes
  e deve ser fechado com a área de negócio antes do desenvolvimento.
- Mapa da rede — o protótipo tem um esquema desenhado à mão. É preciso
  decidir entre tiles pré-descarregados (mapa real, offline, mais trabalho),
  imagem estática do serviço de cartografia da Câmara, ou manter o esquema.
  Falta também saber se as posições dos veículos estão disponíveis e com que
  precisão — sem isso, os autocarros no mapa não passam de decoração.
- Pontos de interesse — quem decide o que entra? Junto de um hospital, uma
  farmácia é serviço público; um café é publicidade não paga. A fronteira
  precisa de um critério escrito.
- Endereço do QR — está a apontar para `tub.pt/app`, que é suposição. Convém
  uma página de redireccionamento que detete o sistema operativo e envie para
  a loja certa, com marcação de origem (`?src=mupi-hospital`) para se conseguir
  medir quantas instalações vieram do equipamento. Sem isso, a funcionalidade
  existe mas ninguém sabe se resulta.

## Passo seguinte natural

Trocar as constantes da secção `[2]` por chamadas `fetch()` a uma API. Se
mantiveres os mesmos nomes de campos, nada mais precisa de mudar. Depois disso,
se o projeto crescer, a migração para React é quase mecânica: cada função da
secção `[4]` vira um componente e o objeto `app` vira estado.
